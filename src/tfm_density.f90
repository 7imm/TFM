...
! ------------------------------------------------------------------------------
  ...
  &  density                                    &
    ...
    REAL(dp), DIMENSION(nz), INTENT(INOUT) :: density
    ...
    REAL(dp), DIMENSION(nz) :: comp_depth

    !---------------------------------------------------------------------------

    comp_depth = -(depth - depth(nz))

    k0 =  11.0_dp * exp(-10160.0_dp / (GAS_CONST * temperature))
    k1 = 575.0_dp * exp(-21400.0_dp / (GAS_CONST * temperature))

    density = (                                            &
    &  (surface_density / (ICE_DENSITY - surface_density)) &
    &  * exp((ICE_DENSITY / 1000.0_dp) * k0 * comp_depth)  &
    )
    density = ICE_DENSITY * (density / (1.0_dp + density))
    ...
    &  * (550.0_dp - density(n+1))                                         &
    )

    density(1:n) = (                            &
    &  (550.0_dp / (ICE_DENSITY - 550.0_dp))    &
    &  * exp(                                   &
    &    (ICE_DENSITY / 1000.0_dp)              &
    &    * (k1 / (accumulation * SECONDS_YEAR)) &
    &    * (comp_depth(1:n) - z550)             &
    &  )                                        &
    )
    density(1:n) = ICE_DENSITY * (density(1:n) / (1.0_dp + density(1:n)))
