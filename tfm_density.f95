...
! ---------------------------------------------------------------------
  ...
  function tfm_density_NabarroHerringCreep(temperature, grain_radius, &
...
! ---------------------------------------------------------------------
    ...
    !else if (temperature <= 263.15_prec) then
    !  rate_factor_dislocation_creep = tfm_density_arrhenius( &
    !  &  1,                                                  &
...
! ----------------------------------------------------------------------
  ...
  function tfm_density_sintering(nz, dt, depth, density, temperature, &
    ...
    real(prec)                :: alt_grain_radius
    real(prec)                :: contact_area
    real(prec), dimension(nz) :: stress
    real(prec), dimension(nz) :: strain_rate

    real(prec), parameter :: AMPLITUDE_GBO = 4.25E-6_prec ! m
    real(prec), parameter :: MU = 0.7_prec
    real(prec), parameter :: AIR_PRESSURE = 101325.0_prec ! Pa
    ...
    do n = 1, nz, 1
      ...
      alt_grain_radius = (                                  &
      &  ((density(n) / 360.0_prec)**(1.0_prec / 3.0_prec)) &
      &  * 0.0005_prec                                      &
      )
      contact_area = tfm_density_arztContactarea( &
      &  1,                                       &
      &  (density(n) / ICE_DENSITY),              &
      &  (359.0_prec / ICE_DENSITY)               &
      )
      contact_area = contact_area * (alt_grain_radius**2.0_prec)
      neck_radius = ((contact_area**0.5_prec) / PI)
