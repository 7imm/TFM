...
accumulation rate needed for the computation are the mean values of the surface
temperature ${\bar{T} = 261.15 \, \mathrm{K}}$ and the solid accumulation rate
...
age profile from the Herron & Langway model was used. The grain radius at the
surface is set to be the time-independent forcing value of
${\rho_0 = 0.5 \, \mathrm{mm}}$.
...
values along the profile. The inital temperature is the mean surface temperature
of the forcing ${T_\mathrm{init} = 261.15 \, \mathrm{K}}$. The initital
volumetric water content is simply set to zero ${\theta_\mathrm{init} = 0.0}$.
...
melting point in the first half of the year. Accordingly, the solid accumuation
becomes negative at that time and the liquid accumulation becomes positive.
...
year when surface temperatures are above the pressure melting point. The same
amount of negative solid accumuation is defined as positive liquid accumulation.
Suggesting that the firn melts at the surface and the generated meltwater is
infiltrated.
