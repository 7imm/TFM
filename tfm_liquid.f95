...
! ----------------------------------------------------------------------
    ...
    &  * (                                                 &
    &    (3.0_prec * (grain_radius**2.0_prec))             &
...
! ----------------------------------------------------------------------
  ...
  function vgSaturationSelfConsistent(nz, density, grain_radius) result(saturation_cond)
    implicit none
    ...
    real(prec), dimension(nz) :: beta

    real(prec), parameter :: DYNAMIC_VISCOSITY = 0.001792_prec


    beta = (1.0_prec - (1.0_prec - (density / ICE_DENSITY)))**(1.0_prec / 3.0_prec)

    saturation_cond = (                                           &
    &  (WATER_DENSITY * ACC_GRAVITY) / DYNAMIC_VISCOSITY          &
    &  * (                                                        &
    &    (grain_radius**2.0_prec / (3.0_prec * (beta**2.0_prec))) &
    &    * (-1.0_prec + (                                         &
    &      (2.0_prec + (3.0_prec * (beta**5.0_prec)))             &
    &      / (beta * (3.0_prec + (2.0_prec * (beta**5.0_prec))))  &
    &    ))                                                       &
    &  )                                                          &
    )
  ...
  function vgSaturationCarmanKozeny(nz, density, grain_radius) result(saturation_cond)
    implicit none
    ...
    &  ((WATER_DENSITY * ACC_GRAVITY) / DYNAMIC_VISCOSITY)       &
    &  * (                                                       &
    &    ((4.0_prec * grain_radius**2.0) * (porosity**3.0_prec)) &
    &    / (180.0_prec * ((1.0_prec - porosity)**2.0_prec))      &
    &  )                                                         &
    )
